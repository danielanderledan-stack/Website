...
(function () {
  ...
  function safe(name, fn) { try { fn(); } catch (e) { /* a broken/edited section must not stop the others */ } }
  ...
  safe('nav-shadow', function () {
    ...
    if (!nav) return;
    var update = function () {
      nav.style.boxShadow = window.scrollY > 30 ? '0 2px 20px rgba(0,0,0,0.6)' : 'none';
    };
  ...
  safe('hero', function () {
    ...
    function show(i) {
      ...
      if (title && sl.getAttribute('data-title')) { title.textContent = sl.getAttribute('data-title'); replay(title, titleAnim); }
      if (sub && sl.getAttribute('data-sub')) { sub.textContent = sl.getAttribute('data-sub'); replay(sub, subAnim); }
      restart();
    }
    function restart() {
  ...
  safe('statbar', function () {
    ...
    bars.forEach(function (bar) {
      ...
      var io = new IntersectionObserver(function (entries) {
        entries.forEach(function (en) {
          if (en.isIntersecting) { bar.style.width = target; io.disconnect(); }
        });
      }, { threshold: 0.5 });
  ...
  safe('calculator', function () {
    ...
    function recompute() {
      ...
      if (badge && range) badge.textContent = range.value + ' ' + (badge.getAttribute('data-unit') || 'km');
      if (select) select.style.color = select.value ? 'var(--color-secondary)' : '#999';
    }
    function wireGroup(wrap, onPick) {
      if (!wrap) return;
      $all('button', wrap).forEach(function (btn, i, all) {
        btn.addEventListener('click', function () {
          all.forEach(function (b) { b.classList.remove('cd-on'); });
    ...
    wireGroup(urgWrap, function (i) { urgent = i === 0; recompute(); });
    wireGroup(typeWrap, function () { /* visual only, like the original */ });
    recompute();
  });
  ...
  safe('newsletter', function () {
    $all('form[data-cd="newsletter"]').forEach(function (form) {
      form.addEventListener('submit', function (e) {
        ...
        p.style.cssText = 'font-family:Roboto;font-size:13px;color:#1bd760;font-weight:700';
  ...
  safe('image-fallback', function () {
    function fb(img) {
      ...
      var w = 600, h = 400, m = (img.src || '').match(/w=(\d+)&h=(\d+)/);
      if (m) { w = m[1]; h = m[2]; } else { w = img.naturalWidth || img.width || 600; h = img.naturalHeight || img.height || 400; }
      if (img.getAttribute('data-fb') !== '1') {
        img.setAttribute('data-fb', '1');
        img.src = 'https://picsum.photos/seed/' + encodeURIComponent(((img.alt || 'img') + w + h).slice(0, 24)) + '/' + w + '/' + h;
      } else {
        img.setAttribute('data-fb', '2');
        img.src = 'data:image/svg+xml;utf8,' + encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" width="' + w + '" height="' + h + '"><rect width="100%" height="100%" fill="#dfe3e8"/></svg>');
      }
    }
